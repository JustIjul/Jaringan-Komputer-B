...
| ----------- | --------- | ------------- | ------------- | ----------- |
| netics-pc-1 | eth0      | 10.98.100.101 | 255.255.255.0 | 10.98.100.1 |
| netics-pc-2 | eth0      | 10.98.100.102 | 255.255.255.0 | 10.98.100.1 |
| netics-pc-4 | eth0      | 10.98.100.104 | 255.255.255.0 | 10.98.100.1 |
| netics-pc-3 | eth0      | -             | -             | -           |
| netics-pc-3 | eth1      | -             | -             | -           |
| netics-pc-3 | eth2      | -             | -             | -           |
| netics-pc-3 | br0       | 10.98.100.1   | 255.255.255.0 | -           |
...
```bash
ip addr add 10.98.100.101/24 dev eth0
ip link set eth0 up
```
...
```bash
ip addr add 10.98.100.102/24 dev eth0
ip link set eth0 up
```
...
```bash
ip addr add 10.98.100.104/24 dev eth0
ip link set eth0 up
```
...
```bash
# Aktifkan interface eth0, eth1, dan eth2
ip link set eth0 up
ip link set eth1 up
ip link set eth2 up
...
# Tambahkan eth0, eth1, dan eth2 ke bridge
...
Bridge (br0) di PC-3 berfungsi sebagai perangkat penghubung layer 2 yang menyatukan interface eth0, eth1, dan eth2. Semua traffic dari PC-1, PC-2, dan PC-4 akan melewati bridge ini, memungkinkan ketiga PC tersebut berkomunikasi dalam satu subnet.
...
```bash
ping -c 5 10.98.100.102
```

**Output:**

```
PING 10.98.100.102 (10.98.100.102) 56(84) bytes of data.
64 bytes from 10.98.100.102: icmp_seq=1 ttl=64 time=0.234 ms
64 bytes from 10.98.100.102: icmp_seq=2 ttl=64 time=0.198 ms
64 bytes from 10.98.100.102: icmp_seq=3 ttl=64 time=0.201 ms
64 bytes from 10.98.100.102: icmp_seq=4 ttl=64 time=0.215 ms
64 bytes from 10.98.100.102: icmp_seq=5 ttl=64 time=0.227 ms

--- 10.98.100.102 statistics ---
...
```
                              My traceroute  [v0.93]
netics-pc-1 (10.98.100.101) -> 10.98.100.102                2024-09-24T05:22:41
Keys:  Help   Display mode   Restart statistics   Order of fields   quit
                                       Packets               Pings
 Host                                Loss%   Snt   Last   Avg  Best  Wrst StDev
 1. 10.98.100.102                      0.0%   100  0.24  0.22  0.19  0.35  0.03
```

**Screenshot:**
![Hasil MTR PC-2](img/netics-pc-2-packet-loss.png)
...
```
                              My traceroute  [v0.93]
netics-pc-1 (10.98.100.101) -> 10.98.100.104                2024-09-24T05:22:41
Keys:  Help   Display mode   Restart statistics   Order of fields   quit
                                       Packets               Pings
 Host                                Loss%   Snt   Last   Avg  Best  Wrst StDev
 1. 10.98.100.104                      0.0%   100  0.28  0.25  0.22  0.38  0.04
```
...
# Terapkan packet loss 20% pada eth2
...
64 bytes from 10.98.100.102: icmp_seq=17 ttl=64 time=0.217 ms
64 bytes from 10.98.100.102: icmp_seq=18 ttl=64 time=0.225 ms
[dst...]
...
**Screenshot:**
![Hasil Ping Packet Loss PC-1](img/netics-pc-1-packet-loss.png)
...
```
                              My traceroute  [v0.93]
netics-pc-1 (10.98.100.101) -> 10.98.100.102                2024-09-24T07:24:43+0000
Keys:  Help   Display mode   Restart statistics   Order of fields   quit
                                       Packets               Pings
 Host                                Loss%   Snt   Last   Avg  Best  Wrst StDev
 1. 10.98.100.102                     20.0%    35  0.6   1.2  0.2   2.6  0.8
```

**Screenshot:**
![Hasil MTR Packet Loss PC-1](img/netics-pc-1-packet-loss.png)
...
# Terapkan pembatasan throughput 50 Mbps pada eth2
...
-----------------------------------------------------------
Accepted connection from 10.98.100.102, port 45678
[  5] local 10.98.100.101 port 5201 connected to 10.98.100.102 port 45678
...
```bash
iperf3 -c 10.98.100.101 -u -b 100M
```

**Output:**

```
Connecting to host 10.98.100.101, port 5201
[  5] local 10.98.100.102 port 45678 connected to 10.98.100.101 port 5201
